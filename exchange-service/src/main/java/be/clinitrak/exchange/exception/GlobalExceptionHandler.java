...
/**
 * Gestionnaire global des exceptions REST pour le exchange-service.
 *
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Gère les ressources Exchange non trouvées.
     *
     * @param ex      exception ExchangeNotFoundException
    ...
    public ResponseEntity<ProblemDetail> handleExchangeNotFoundException(ExchangeNotFoundException ex, WebRequest request) {
        log.warn("Ressource Exchange non trouvée : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("exchange-not-found"));
    ...
    /**
     * Gère les violations de règles métier Exchange (email dupliqué, statut invalide).
     *
     * @param ex      exception ExchangeException
     * @param request requête web courante
     * @return 400 Bad Request avec ProblemDetail
     */
    @ExceptionHandler(ExchangeException.class)
    public ResponseEntity<ProblemDetail> handleExchangeException(ExchangeException ex, WebRequest request) {
        log.warn("Erreur métier Exchange : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("exchange-business-error"));
