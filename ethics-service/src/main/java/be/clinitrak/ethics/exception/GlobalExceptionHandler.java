...
/**
 * Gestionnaire global des exceptions REST pour le ethics-service.
 *
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Gère les ressources CE non trouvées.
     *
     * @param ex      exception EthicsNotFoundException
     * @param request requête web courante
     * @return 404 Not Found avec ProblemDetail
     */
    @ExceptionHandler(EthicsNotFoundException.class)
    public ResponseEntity<ProblemDetail> handleEthicsNotFoundException(EthicsNotFoundException ex, WebRequest request) {
        log.warn("Ressource CE non trouvée : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("ethics-not-found"));
    ...
    /**
     * Gère les violations de règles métier CE (numéro dupliqué, transition invalide, tenant manquant).
     *
     * @param ex      exception EthicsException
     * @param request requête web courante
     * @return 400 Bad Request avec ProblemDetail
     */
    @ExceptionHandler(EthicsException.class)
    public ResponseEntity<ProblemDetail> handleEthicsException(EthicsException ex, WebRequest request) {
        log.warn("Erreur métier CE : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("ethics-business-error"));
