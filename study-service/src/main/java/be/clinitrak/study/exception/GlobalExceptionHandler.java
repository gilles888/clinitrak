...
/**
 * Gestionnaire global des exceptions REST pour le study-service.
 *
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Gère les études non trouvées.
     *
     * @param ex      exception StudyNotFoundException
     * @param request requête web courante
     * @return 404 Not Found avec ProblemDetail
     */
    @ExceptionHandler(StudyNotFoundException.class)
    public ResponseEntity<ProblemDetail> handleStudyNotFoundException(StudyNotFoundException ex, WebRequest request) {
        log.warn("Étude non trouvée : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("study-not-found"));
        problem.setTitle("Étude clinique introuvable");
        problem.setProperty("timestamp", Instant.now());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(problem);
    }

    /**
     * Gère les violations de règles métier (code dupliqué, transition invalide, tenant manquant).
     *
     * @param ex      exception StudyException
     * @param request requête web courante
     * @return 400 Bad Request avec ProblemDetail
     */
    @ExceptionHandler(StudyException.class)
    public ResponseEntity<ProblemDetail> handleStudyException(StudyException ex, WebRequest request) {
        log.warn("Erreur métier : {}", ex.getMessage());
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.getMessage());
        problem.setType(PROBLEM_BASE_URI.resolve("study-business-error"));
